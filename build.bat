...
:BUILD_START
echo.
echo Building PARRY Paranoid Chatbot...
echo.
...
if exist parry_univac.exe del /Q parry_univac.exe
if exist main_univac.o del /Q main_univac.o
if exist parry_univac.o del /Q parry_univac.o
if exist *.o del /Q *.o
echo.
...
gcc -c -DUNIVAC -O2 -Wall -std=c99 main.c -o main_univac.o
...
echo Compiling parry.c...
gcc -c -DUNIVAC -O2 -Wall -std=c99 parry.c -o parry_univac.o

if %ERRORLEVEL% NEQ 0 (
    echo ERROR: Failed to compile parry.c
    pause
    exit /b 1
)

echo Linking...
gcc -o parry_univac.exe main_univac.o parry_univac.o
...
if %ERRORLEVEL% EQU 0 (
    ...
    echo ========================================
    echo.
    echo Output: parry_univac.exe

    REM Display file size
    for %%A in (parry_univac.exe^) do (
...
REM Clean previous build artifacts
echo Cleaning previous build artifacts...
if exist parry_mingw.exe del /Q parry_mingw.exe
if exist main.obj del /Q main.obj
if exist parry.obj del /Q parry.obj
if exist *.o del /Q *.o
echo.
...
gcc %WARNING_FLAGS% %OPTIMIZE_FLAGS% %PERF_FLAGS% -std=c99 ^
    -o parry_mingw.exe ^
    main.c parry.c ^
    %LINKER_FLAGS%
...
if %ERRORLEVEL% EQU 0 (
    ...
    echo ========================================
    echo.
    echo Output: parry_mingw.exe

    REM Display file size
    for %%A in (parry_mingw.exe) do (
        echo File size: %%~zA bytes
    )
    echo.
    echo Optimization level: MAXIMUM (-O3 + LTO + native CPU^)
    echo.
    echo To run PARRY, type: parry_mingw.exe
    echo.
) else (
    echo.
    echo ========================================
...
cl /W4 /O2 /Fe:parry.exe main.c parry.c
...
if %ERRORLEVEL% EQU 0 (
    ...
    echo ========================================
    echo.
    echo Output: parry.exe
    echo.
    echo To run: parry.exe
) else (
    echo.
    echo ========================================
    echo   BUILD FAILED
